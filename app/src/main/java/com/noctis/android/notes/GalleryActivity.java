...
import com.noctis.android.notes.models.views.InterceptorFrameLayout;
...
public class GalleryActivity extends AppCompatActivity {
  ...
  OnViewTouchedListener screenTouches = new OnViewTouchedListener() {
    ...
    private void click () {
      ...
      if (attachment.getMime_type().equals(ConstantsBase.MIME_TYPE_VIDEO)) {
        viewMedia();
      }
    }
  };
  ...
  private void initData () {
    String title = getIntent().getStringExtra(ConstantsBase.GALLERY_TITLE);
    images = getIntent().getParcelableArrayListExtra(ConstantsBase.GALLERY_IMAGES);
    int clickedImage = getIntent().getIntExtra(ConstantsBase.GALLERY_CLICKED_IMAGE, 0);
    ...
    if (images.get(clickedImage).getMime_type().equals(ConstantsBase.MIME_TYPE_VIDEO)) {
      viewMedia();
    }
  }
