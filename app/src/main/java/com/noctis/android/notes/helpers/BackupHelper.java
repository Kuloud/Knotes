...
import static com.noctis.android.notes.utils.ConstantsBase.DATABASE_NAME;
...
public class BackupHelper {
  ...
  public static boolean exportAttachments (NotificationsHelper notificationsHelper, File destinationattachmentsDir,
    ...
    for (Attachment attachment : list) {
      try {
        StorageHelper.copyToBackupDir(destinationattachmentsDir, FilenameUtils.getName(attachment.getUriPath()),
            NotesApp.getAppContext().getContentResolver().openInputStream(attachment.getUri()));
        ++exported;
      ...
      String failedString = failed > 0 ? " (" + failed + " " + NotesApp.getAppContext().getString(R.string.failed) + ")" : "";
      if (notificationsHelper != null) {
        notificationsHelper.updateMessage(
            TextHelper.capitalize(NotesApp.getAppContext().getString(R.string.attachment))
    ...
    Observable.from(listOld)
              ...
              .forEach(attachment -> StorageHelper.delete(NotesApp.getAppContext(), new File
  ...
  public static void startBackupService (String backupFolderName) {
    Intent service = new Intent(NotesApp.getAppContext(), DataBackupIntentService.class);
    ...
    NotesApp.getAppContext().startService(service);
  }
  ...
  public static boolean exportSettings (File backupDir) {
    File preferences = StorageHelper.getSharedPreferencesFile(NotesApp.getAppContext());
