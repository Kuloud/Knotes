...
public class AttachmentTask extends AsyncTask<Void, Void, Attachment> {
  ...
  protected Attachment doInBackground (Void... params) {
    Attachment attachment = StorageHelper.createAttachmentFromUri(NotesApp.getAppContext(), uri);
  ...
  protected void onPostExecute (Attachment mAttachment) {
    if (isAlive()) {
      ...
      if (mAttachment != null) {
        StorageHelper.delete(NotesApp.getAppContext(), mAttachment.getUri().getPath());
      }
    }
  }
