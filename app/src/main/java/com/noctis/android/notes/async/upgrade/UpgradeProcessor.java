...
package com.noctis.android.notes.async.upgrade;
 ...
 * strictly related to DB (for this {@link DbHelper#onUpgrade(android.database.sqlite.SQLiteDatabase,
    ...
    for (Attachment attachment : dbHelper.getAllAttachments()) {
      if (attachment.getMime_type() == null) {
        ...
        if (!TextUtils.isEmpty(mimeType)) {
          ...
          switch (type) {
            case "image":
              attachment.setMime_type(ConstantsBase.MIME_TYPE_IMAGE);
              break;
            case "video":
              attachment.setMime_type(ConstantsBase.MIME_TYPE_VIDEO);
              break;
            case "audio":
              attachment.setMime_type(ConstantsBase.MIME_TYPE_AUDIO);
              break;
            default:
    ...
    for (Attachment attachment : dbHelper.getAllAttachments()) {
      if ("audio/3gp".equals(attachment.getMime_type()) || "audio/3gpp".equals(attachment.getMime_type
        ...
        File to = new File(from.getParent(), from.getName().replace(FilenameUtils.getExtension(from
            .getName()), ConstantsBase.MIME_TYPE_AUDIO_EXT));
        from.renameTo(to);
