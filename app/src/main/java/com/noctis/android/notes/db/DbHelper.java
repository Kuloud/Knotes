 ...
 */
package com.noctis.android.notes.db;
...
import com.noctis.android.notes.async.upgrade.UpgradeProcessor;
import com.noctis.android.notes.exceptions.DatabaseException;
...
import com.noctis.android.notes.models.Stats;
import com.noctis.android.notes.models.Tag;
import com.noctis.android.notes.utils.AssetUtils;
...
import com.noctis.android.notes.utils.Security;
...
public class DbHelper extends SQLiteOpenHelper {
  ...
  public static synchronized DbHelper getInstance () {
    return getInstance(NotesApp.getAppContext());
  }
  ...
  public static synchronized DbHelper getInstance (boolean forcedNewInstance) {
    if (instance == null || forcedNewInstance) {
      Context context = (instance == null || instance.mContext == null) ? NotesApp.getAppContext() : instance.mContext;
      instance = new DbHelper(context);
    }
    return instance;
  }
  ...
  public Note updateNote (Note note, boolean updateLastModification) {
    ...
    String content = Boolean.TRUE.equals(note.isLocked())
        ? Security.encrypt(note.getContent(), prefs.getString(ConstantsBase.PREF_PASSWORD, ""))
        : note.getContent();
  ...
  public List<Note> getNotes (String whereCondition, boolean order) {
    ...
    if (Navigation.checkNavigation(Navigation.REMINDERS)) {
      ...
      sortColumn = prefs.getString(ConstantsBase.PREF_SORTING_COLUMN, KEY_TITLE);
    }
    if (order) {
    ...
    sortColumn = KEY_REMINDER.equals(sortColumn) ? "IFNULL(" + KEY_REMINDER + ", " +
        "" + ConstantsBase.TIMESTAMP_UNIX_EPOCH + ")" : sortColumn;
      ...
      if (cursor.moveToFirst()) {
        do {
          ...
          if (Boolean.TRUE.equals(note.isLocked())) {
            note.setContent(Security.decrypt(note.getContent(), prefs.getString(ConstantsBase.PREF_PASSWORD, "")));
          }
  ...
  public List<Note> getNotesByCategory (Long categoryId) {
    List<Note> notes;
    boolean filterArchived = prefs.getBoolean(ConstantsBase.PREF_FILTER_ARCHIVED_IN_CATEGORIES + categoryId, false);
    try {
      String whereCondition = " WHERE "
